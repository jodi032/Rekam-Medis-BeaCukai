...
<head>
	...
	if (isset($_POST['submit'])) {
		...
		$new_pass = $_POST['new_password'];
				...
				$up2 = mysqli_query($conn, "UPDATE pegawai SET nama_pegawai='$nama', username='$user', password='$new_pass', pekerjaan='$job',  alamat='$alam' WHERE id='$id'");
				echo '<script>
				setTimeout(function() {
					swal({
					title: "Data Diubah",
							...
							<div class="col-12">
								<div class="card">
									<div class="card-header">
										...
										<a href="printusers.php" class="btn btn-warning">Print</a>
											<a href="#" class="btn btn-primary" data-target="#addUser" data-toggle="modal">Tambah Users</a>
										</div>
									</div>
									<div class="card-body">
													...
													?>
														<tr>
															...
															<td><?php
																if ($row['pekerjaan'] == '1') 
																{
																	echo '<div class="badge badge-pill badge-primary mb-1">Kepala Bea Cukai';
																} 
																if ($row['pekerjaan'] == '2') 
																{
																	echo '<div class="badge badge-pill badge-success mb-1">Pegawai';
																}
																if ($row['pekerjaan'] == '3') 
																{
																	echo '<div class="badge badge-pill badge-warning mb-1">Dokter';
															    }
																if ($row['pekerjaan'] == '4') 
																{
																	echo '<div class="badge badge-pill badge-danger mb-1">Admin';
																} ?>
										</div>
										</td>
										<td>
											<span data-target="#editUser" data-toggle="modal" data-id="<?php echo $row['id']; ?>" data-nama="<?php echo $row['nama_pegawai']; ?>" data-user="<?php echo $row['username']; ?>" data-alam="<?php echo $row['alamat']; ?>" data-pekerjaan="<?php echo $row['pekerjaan']; ?>">
						...
						<form action="" method="POST" class="needs-validation" novalidate="">
							...
							<div class="form-group">
								...
								<select class="form-control selectric" name="pekerjaan">
									<option value="1">Kepala Bea Cukai</option>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<form action="" method="POST" class="needs-validation" novalidate="">
							...
							<div class="form-group">
								<label>Pekerjaan</label>
								<select class="form-control selectric" name="pekerjaan" id="getpekerjaan">
									<option value="1">Kepala Bea Cukai</option>
									<option value="2">Pegawai</option>
									<option value="3">Dokter</option>
									<option value="4">Admin</option>
							...
							<div class="alert alert-danger text-center">
								Jika password tidak diganti, mohon di isi kolom password baru dengan memasukkan password yang lama.
								Jika Password benar diganti silahkan isi password baru di kolom password baru.
							</div>
							<div class="form-group row">
